model pipeValidation
  ...
  Modelica.Fluid.Pipes.DynamicPipe pipe(redeclare package Medium = Medium, diameter = 0.022,  length = 5, modelStructure = Modelica.Fluid.Types.ModelStructure.av_vb, roughness (displayUnit = "m") = 1.1e-05) annotation(
  ...
  Modelica.Blocks.Sources.Ramp ramp(duration = 60, height = 60 / 60) annotation(
  ...
  Modelica.Blocks.Sources.RealExpression a(y = (-8 / pi ^ 2) * rho * (1 / d.y ^ 4) * (f.y / d.y)) annotation(
  ...
  Modelica.Blocks.Sources.RealExpression v(y = 0.5) annotation(
  ...
  Modelica.Blocks.Sources.RealExpression epsilon(y = 1.1e-05) annotation(
  ...
  Modelica.Blocks.Sources.RealExpression dotV_LperMin(y = pipe.port_a.m_flow * 60)  annotation(
    Placement(visible = true, transformation(origin = {-90, -34}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression dotV_LperMin_dummy(y = pipe.port_a.m_flow * 60) annotation(
...
equation
 ...
 Reynolds = v.y*d.y/nu;
  ...
  connect(epsilon.y, relRou.u1) annotation(
    Line(points = {{-18, 54}, {-8, 54}}, color = {0, 0, 127}));
  connect(relRou.u2, d.y) annotation(
    Line(points = {{-8, 42}, {-14, 42}, {-14, 84}, {-18, 84}}, color = {0, 0, 127}));
  annotation(
    uses(Modelica(version = "3.2.3")),
    Diagram(graphics = {Text(origin = {-58, 98}, extent = {{-16, 4}, {16, -4}}, textString = "pipe.vs[1]"), Text(origin = {4, 117}, extent = {{-34, 9}, {34, -9}}, textString = "DetailedFlow Model")}));
end pipeValidation;
